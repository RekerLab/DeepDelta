...
for dataset in datasets:
  ...
  for i in range(10):
    ...
    n += n_fold
    diagonal = np.append(np.array(diagonal), np.diag(matrix))
    reverse_paired = np.append(np.array(reverse_paired), (matrix + np.transpose(matrix)).flatten())
    ...
    triangle_paired =  np.append(np.array(triangle_paired), triangle_paired_fold)
