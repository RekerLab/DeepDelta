...
class Delta_ChemProp(abstractDeltaModel):
    ...
    def predict_delta_reverse(self, x): 
...
def cross_validation(x, y, model, k=10, iter=1): 
...
def cross_validation_file(data_path, model, k=10, iter=1):
  ...
  x = df[df.columns[0]]
  y = df[df.columns[1]]
  return cross_validation(x,y,model,k,iter) 


def cross_validation_reverse(x, y, model, k=10, iter=1): 
  kf = KFold(n_splits=k, random_state=iter, shuffle=True)
...
for property in properties:
    i = 1 # Using a random state of 1 for this analysis
    dataset = '{}.csv'.format(property)
