...
all_scoresNM = pd.DataFrame(columns=['Dataset', 'Pearson\'s r RF', 'MAE RF', 'RMSE RF',
                                    'Pearson\'s r CP', 'MAE CP', 'RMSE CP', 'Pearson\'s r DD', 'MAE DD', 'RMSE DD']) # Pairs of Nonmatching Scaffolds
all_scoresM = pd.DataFrame(columns=['Dataset', 'Pearson\'s r RF', 'MAE RF', 'RMSE RF',
                                    'Pearson\'s r CP', 'MAE CP', 'RMSE CP', 'Pearson\'s r DD', 'MAE DD', 'RMSE DD']) # Pairs with Matching Scaffolds

for property in properties:
    dataset = '{}.csv'.format(property)

    # 3 Options of Datasets 
    # 1 - ChemProp
    predictions_CP = pd.read_csv('{}_DeepDelta_50_Epoch_Trad_1.csv'.format(name)).T
    predictions_CP.columns =['True', 'Delta']

    # 2 - DeepDelta
    predictions_DD = pd.read_csv('{}_DeepDelta_1.csv'.format(name)).T
    predictions_DD.columns =['True', 'Delta', 'Traditional']

    # 3 - Random Forest 
    predictions_RF = pd.read_csv('{}_CV_RF_1.csv'.format(name))
    predictions_RF.columns =['Ignore', 'True', 'Delta']
    ...
    all_scoresNM = all_scoresNM.append(scoringNM)
    ...
    all_scoresM = all_scoresM.append(scoringM)

all_scoresNM.to_csv("DeepDelta_Scaffold_NM.csv", index = False)
all_scoresM.to_csv("DeepDelta_Scaffold_M.csv", index = False)