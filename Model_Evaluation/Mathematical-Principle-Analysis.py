...
for dataset in datasets:
  ...
  # Cross validation
  cv = KFold(n_splits=10, random_state=1, shuffle=True) # Used a random state of 1 for these analysis
  datapoint_x = []
  datapoint_y = []
...
##############################################
### Eq.2: DeepDelta(x,y) = -DeepDelta(y,x) ###
##############################################
...
for dataset in datasets:
    ...
    scoring2 = pd.DataFrame({'Dataset': [name], 'Pearson\'s r': [round(pearson[0], 4)], 'MAE': [round(MAE, 4)], 'RMSE': [round(RMSE, 4)]})
    scores = scores.append(scoring2)
...
##############################################################
### Eq.3: DeepDelta(x,y) + DeepDelta(y,z) = DeepDelta(x,z) ###
##############################################################
